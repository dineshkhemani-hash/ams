...
//        }
    ...
    @GetMapping("/{userId}")
    public ResponseEntity<?> getAttendanceByUser(@PathVariable UUID userId, HttpServletRequest request){
        Users currentUser = AuthenticationService.getCurrentUser();
        if(currentUser != null){
            List<AttendanceListDTO> attendanceList = adminAttendanceService.getAttendanceByUserId(userId);
